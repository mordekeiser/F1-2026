...
const FLAG: Record<string, string> = {
  ...
  Azerbaijan:"🇦🇿",Singapore:"🇸🇬",Mexico:"🇲🇽",Brazil:"🇧🇷","Abu Dhabi":"🇦🇪",Qatar:"🇶🇦",
};

export default function LastRaceResults({ initial }: { initial: Race | null }) {
  const [race, setRace] = useState<Race | null>(initial);

  useEffect(() => {
    const refresh = async () => {
      try {
        const res = await fetch("/api/f1?type=lastrace");
      ...
      <div className="max-w-4xl mx-auto px-6">
        <div className="mb-8">
          ...
          <p className="text-zinc-500 text-sm mt-1">
            {flag} Round {race.round} · {race.raceName} · {new Date(race.date).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            { pos: "2", result: race.Results[1], emoji: "🥈", h: "h-24", medal: "bg-zinc-300 text-black" },
            { pos: "1", result: race.Results[0], emoji: "🥇", h: "h-32", medal: "bg-yellow-400 text-black" },
            { pos: "3", result: race.Results[2], emoji: "🥉", h: "h-20", medal: "bg-amber-600 text-white" },
          ].map(({ pos, result, emoji, h, medal }) => {
            if (!result) return null;
            const color = getTeamColor(result.Constructor.name);
            return (
              <motion.div key={pos}
                initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }} transition={{ delay: Number(pos) * 0.1 }}
            ...
            return (
              <motion.div key={r.position}
                ...
                <div className="flex-1">
                  <div className="text-sm font-bold text-white">{r.Driver.givenName} {r.Driver.familyName}</div>
                  <div className="text-xs text-zinc-500">{r.Constructor.name}</div>
                </div>
                <div className="text-sm font-black" style={{ color }}>{r.points} pts</div>
                {r.FastestLap?.rank === "1" && (
                  <span className="text-xs bg-purple-900/40 text-purple-400 px-2 py-0.5 rounded-full font-bold">⚡ FL</span>
                )}
              </motion.div>
            );
          })}
