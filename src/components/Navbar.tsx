...
const links = [
  {label:"Home",href:"#home"},{label:"Schedule",href:"#schedule"},
  {label:"Standings",href:"#standings"},{label:"Circuits",href:"#circuits"},
  {label:"Drivers",href:"#drivers"},{label:"Teams",href:"#teams"},
    ...
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 border-b border-[#e10600] ${scrolled?"bg-black/95 backdrop-blur-md":"bg-black/80 backdrop-blur-sm"}`}>
      <div className="max-w-7xl mx-auto px-5 py-3 flex items-center justify-between">
        <a href="#home" className="flex items-center gap-2">
          <span className="text-[#e10600] text-xl font-black">F1</span>
          <span className="text-white text-xl font-bold">NEXUS</span>
          <span className="text-xs text-zinc-500 mt-0.5">2026</span>
        </a>
        <div className="hidden md:flex items-center gap-1">
          {links.map(l=>(
            <a key={l.href} href={l.href} className="px-3 py-1.5 rounded-md text-sm font-medium text-zinc-400 hover:text-white hover:bg-white/5 transition-all">{l.label}</a>
          ))}
        </div>
      ...
      <AnimatePresence>
        {open&&(
          <motion.div initial={{height:0,opacity:0}} animate={{height:"auto",opacity:1}} exit={{height:0,opacity:0}} className="md:hidden overflow-hidden bg-black border-t border-zinc-800">
            {links.map(l=>(
              <a key={l.href} href={l.href} onClick={()=>setOpen(false)} className="block px-6 py-3 text-sm text-zinc-400 hover:text-white border-b border-zinc-900">{l.label}</a>
            ))}
          </motion.div>
        )}
      </AnimatePresence>
